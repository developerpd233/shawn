    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.transactions.update", [$transaction->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="orders">{{ trans('cruds.transaction.fields.order') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('orders') ? 'is-invalid' : '' }}" name="orders[]" id="orders" multiple required>
                    @foreach($orders as $id => $order)
                        <option value="{{ $id }}" {{ (in_array($id, old('orders', [])) || $transaction->orders->contains($id)) ? 'selected' : '' }}>{{ $order }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label class="required" for="transaction">{{ trans('cruds.transaction.fields.transaction') }}</label>
                <input class="form-control {{ $errors->has('transaction') ? 'is-invalid' : '' }}" type="text" name="transaction" id="transaction" value="{{ old('transaction', $transaction->transaction) }}" required>
