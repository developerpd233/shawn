    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.chats.update", [$chat->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('buyers') ? 'is-invalid' : '' }}" name="buyers[]" id="buyers" multiple required>
                    @foreach($buyers as $id => $buyer)
                        <option value="{{ $id }}" {{ (in_array($id, old('buyers', [])) || $chat->buyers->contains($id)) ? 'selected' : '' }}>{{ $buyer }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('message') ? 'is-invalid' : '' }}" type="text" name="message" id="message" value="{{ old('message', $chat->message) }}">
